            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div>
                    <h2 className="text-4xl lg:text-5xl font-black text-white tracking-tighter mb-2">
                        {project.name} <span className="text-indigo-400">Venture</span>
                    </h2>
                    ...
                    return (
                        <div key={milestone.id} className="bg-[#1a1a1a]/40 rounded-[40px] border border-white/5 overflow-hidden backdrop-blur-3xl shadow-2xl relative group">
                            <div className="p-8 border-b border-white/5 bg-white/0 flex justify-between items-center relative z-10">
                                <div>
                                    <h3 className="text-2xl font-black text-white tracking-tight mb-2 group-hover:text-indigo-400 transition-colors">{milestone.name}</h3>
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-3 gap-0 divide-y lg:divide-y-0 lg:divide-x divide-white/5 relative z-10">
                                    ...
                                    <div className="space-y-4">
                                        {isAddingTask === milestone.id && (
                                            <div className="bg-[#1a1a1a]/60 p-6 rounded-[28px] border border-white/10 backdrop-blur-3xl shadow-2xl relative overflow-hidden animate-in fade-in slide-in-from-top-4 duration-300">
                                                <div className="absolute inset-x-0 top-0 h-1 bg-gradient-to-r from-indigo-500/50 to-purple-500/50"></div>
                                                <form onSubmit={(e) => handleAddTask(milestone.id, e)} className="space-y-4">
                                                    <div className="space-y-1">
                                                        ...
                                                        <input
                                                            ...
                                                            className="w-full px-4 py-3 bg-white/5 border border-white/5 rounded-xl text-white placeholder-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:bg-white/10 transition-all text-sm font-medium"
                                                            autoFocus
                                                            required
                                                        />
                                                    </div>
                                                    <div className="space-y-1">
                                                        ...
                                                        <select
                                                            ...
                                                            className="w-full px-4 py-3 bg-white/5 border border-white/5 rounded-xl text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:bg-white/10 transition-all text-xs font-medium appearance-none"
                                                        >
                                                    ...
                                                    <div className="flex justify-end gap-3 pt-2">
                                                        ...
                                                        <button
                                                            type="submit"
                                                            className="px-6 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl transition-all duration-300 shadow-lg shadow-indigo-600/20 font-bold uppercase tracking-widest text-[9px]"
                                                        >
                                                            Launch Task
                                                        </button>
                                                    </div>
                                        ...
                                        {todoTasks.map(task => (
                                            <div key={task.id} className="group/task bg-white/5 p-6 rounded-[24px] border border-white/5 hover:border-indigo-500/30 transition-all duration-300 hover:bg-white/[0.08]">
                                                ...
                                                <div className="flex items-center justify-between pt-4 border-t border-white/5">
                                                    ...
                                                    <button
                                                        ...
                                                        className="p-2 bg-indigo-500/10 text-indigo-400 rounded-xl border border-indigo-500/20 hover:bg-indigo-500 hover:text-white transition-all"
                                                    >
                                ...
                                <div className="p-8 bg-indigo-500/[0.02]">
                                    <div className="flex justify-between items-center mb-10">
                                        <div className="flex items-center gap-3">
                                            <div className="w-1.5 h-1.5 rounded-full bg-indigo-500 shadow-[0_0_10px_rgba(79,70,229,0.5)]"></div>
                                            <h4 className="text-[11px] font-black text-white uppercase tracking-[0.3em]">Active Operations</h4>
                                        </div>
                                        <span className="px-3 py-1 bg-indigo-500/10 rounded-full text-[10px] font-black text-indigo-400 border border-indigo-500/20">{inProgressTasks.length}</span>
                                    </div>
                                    <div className="space-y-4">
                                        {inProgressTasks.map(task => (
                                            <div key={task.id} className="group/task bg-[#1a1a1a]/40 p-6 rounded-[24px] border border-white/5 hover:border-indigo-500/30 transition-all duration-300 shadow-xl">
                                                <div className="flex justify-between items-start mb-4">
                                                    <p className="text-white font-bold leading-relaxed">{task.title}</p>
                                                    <button className="text-slate-600 hover:text-white transition-colors">
                                                        <MoreHorizontal className="w-4 h-4" />
