...
const TABLE_CONFIGS: Record<TableName, TableConfig> = {
    ...
    milestones: {
        mapper: (m: any) => ({ id: m.id, projectId: m.project_id, name: m.name, targetDate: m.target_date, completedPercentage: m.completed_percentage, createdAt: m.created_at, clientStatus: m.client_status || 'pending' }),
        orderBy: 'created_at', ascending: false,
    },
    tasks: {
        mapper: (t: any) => ({ id: t.id, projectId: t.project_id, milestoneId: t.milestone_id, engineerId: t.engineer_id, title: t.title, description: t.description, status: t.status, createdAt: t.created_at, startDate: t.start_date, dueDate: t.due_date, duration: t.duration, dependencies: t.dependencies, progress: t.progress, priority: t.priority || 'medium', isAutoGenerated: t.is_auto_generated }),
        orderBy: 'created_at', ascending: false,
    },
    leave_requests: {
...
export const DataProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const addEntry = async (entry: LogEntry) => {
        ...
        // Automated RFI/Revision Watcher
        if (entry.entryType === 'rfi' || entry.entryType === 'revision') {
            const task: Task = {
                id: crypto.randomUUID(),
                projectId: entry.projectId,
                engineerId: entry.engineerId,
                title: `${entry.entryType.toUpperCase()}: ${entry.taskDescription.slice(0, 40)}...`,
                description: `Auto-generated from activity log: ${entry.taskDescription}`,
                status: 'not_started',
                priority: 'high',
            ...
            void addTask(task);
            toast.success(`Priority ${entry.entryType} task auto-created`);
        }
    ...
    const updateMilestone = async (milestone: Milestone) => {
        const { error } = await supabase.from('milestones').update({
            ...
            client_status: milestone.clientStatus
        }).eq('id', milestone.id);
    ...
    const addTask = async (task: Task) => {
        ...
        const { error } = await supabase.from('tasks').insert({
            ...
            duration: task.duration, dependencies: task.dependencies, progress: task.progress || 0
        });
        if (error) {
    ...
    const updateTask = async (task: Task) => {
        ...
        const { error } = await supabase.from('tasks').update({
            ...
            duration: task.duration, dependencies: task.dependencies, progress: task.progress
        }).eq('id', task.id);
        if (error) {
